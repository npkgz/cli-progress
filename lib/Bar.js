...
function Bar(opt, preset){
    ...
    // custom keys and default value for render 
...
Bar.prototype.render = function(){
    ...
    // assign custom placeholder tokens
    for (var key in this.custom) {
        var pattern = RegExp('{'+key+'}', 'gi');
        s = s.replace(pattern, this.custom[key]);
    }
...
Bar.prototype.update = function(current, custom){
    // update value
    this.value = current;
    ...
    for (var key in customData){
        this.custom[key] = customData[key];
    }
