...
module.exports = class GenericBar{
    ...
    render(){
        ...
        const percentage =  padStart(Math.round(progress*100) + '', 3);

        // calculate elapsed time
        const elapsedTime = Math.round((Date.now() - this.startTime)/1000);
        let elapsedTimef;
        if(this.options.formatTime) elapsedTimef = this.options.formatTime(elapsedTime);
        else elapsedTimef = GenericBar.formatTime(elapsedTime, 1);

        // calculate eta
        const eta = this.eta.getTime();
        let etaf;
        if(this.options.formatTime) etaf = this.options.formatTime(etaf);
        else  etaf = GenericBar.formatTime(eta, 5, this.options.formatTime);
